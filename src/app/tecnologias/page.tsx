    ...
    <div className="max-w-4xl mx-auto px-6 py-24 space-y-16">
        ...
        {techCategories.map((cat, i) => (
          ...
          >
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="text-sm font-mono font-semibold text-green uppercase tracking-widest">{cat.name}</h2>
              <div className="h-px flex-1 bg-border" />
            </div>
            <div className="space-y-4">
              {cat.items.map((item) => (
                <div key={item.name} className="bg-card border border-border rounded-xl p-5 hover:border-green/20 transition-colors">
      ...
      <div className="animate-fade-up bg-surface border border-border rounded-xl p-6" style={{ animationDelay: "700ms" }}>
        ...
        <div className="flex flex-wrap gap-2">
          {["Python", "Java", "C++", "Kotlin", "PHP", "Jetpack Compose", "Neo4j", "MySQL", "SQL Server", "WordPress", "WebSockets"].map((tech) => (
